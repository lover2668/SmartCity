 ...
 * Function:FrameMultiUrl 以简洁的 Api,让 Retrofit 不仅支持多 BaseUrl
 * 还可以在 App 运行时动态切换任意 BaseUrl,在多 BaseUrl 场景下也不会影响到其他不需要切换的 BaseUrl
 * Description:设置支持多BaseUrl
 * 1、2017-11-24 14:40:06 JenkinsZhou 新增部分set put方法返回本对象以便链式调用
...
class FrameMultiUrl {
    ...
    private static final String TAG = "FrameMultiUrl";
    ...
    public Request processRequest(Request request) {
        ...
        if (null != httpUrl) {
            ...
            LoggerManager.i(FrameMultiUrl.TAG, "Header 模式重定向Url:Base Url is { " + mBaseUrl + " }" + ";New Url is { " + newUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(newUrl)
                    .build();
        }
        ...
        if (null != httpUrl) {
            LoggerManager.i(FrameMultiUrl.TAG, "Method 模式重定向Url:Base Url is { " + mBaseUrl + " }" + ";New Url is { " + httpUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpUrl)
                    .build();
        }
        ...
        if (httpUrlBase != null && !httpUrlBase.toString().equals(mBaseUrl)) {
            ...
            LoggerManager.i(FrameMultiUrl.TAG, "重定向Url:Base Url is { " + httpUrlBase.toString() + " }" + ";New Url is { " + httpNew + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpNew)
                    .build();
        }
    ...
    private HttpUrl checkUrl(String url) {
        ...
        if (null == parseUrl) {
            throw new NullPointerException(GlobalConstant.EXCEPTION_EMPTY_URL);
        } else {
            return parseUrl;
        }
    }
